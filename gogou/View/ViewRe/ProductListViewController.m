...
-(void)refreshList
{
    ...
    [SVProgressHUD show];
    NSURL *url = [[NSURL alloc] initWithString:PRODUCT_LIST];
    
    __weak  ASIFormDataRequest *request = [ASIFormDataRequest requestWithURL:url];
    NSLog(@"%@==%@==%@==%@\n",self.filterMarketId,self.filterManufacturerId,self.filterName ,self.filterCategoryId );
    [request setRequestMethod:@"POST"];
    [request setPostValue:self.filterMarketId           forKey:FILTER_MARKET_ID ];
    [request setPostValue:self.filterManufacturerId     forKey:FILTER_MANUFACTURER_ID];
    [request setPostValue:self.filterName               forKey:FILTER_NAME];
    [request setPostValue:self.filterCategoryId         forKey:FILTER_CATEGORY_ID];
    [request setPostValue:nil                           forKey:FILTER_SPECIAL];
    [request setPostValue:nil                           forKey:FILTER_SPECIAL_PRO];
    ...
    [request setCompletionBlock:^{
        ...
        for (int i = 0; i < array.count; i ++) {
            ...
            NSString *string=[NSString stringWithFormat:@"%@",infor];
            NSLog(@"==%@==",infor);
