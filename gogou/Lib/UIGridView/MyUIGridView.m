...
{
	MyUIGridViewCell* temp = tempCell;
	tempCell = nil;
	return temp;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    MyUIGridViewRow *row = (MyUIGridViewRow *)[tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    if (row == nil) {
        row = [[MyUIGridViewRow alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier];
    }
	...
	for (int i=0;i<numCols;i++) {
		...
		if ((i + indexPath.row * numCols) >= count) {
			...
			if ([row.contentView.subviews count] > i) {
				((MyUIGridViewCell *)[row.contentView.subviews objectAtIndex:i]).hidden = YES;
			}
		...
		MyUIGridViewCell *cell = [uiGridViewDelegate gridView:self
...
{
	MyUIGridViewCell *cell = (MyUIGridViewCell *) sender;
